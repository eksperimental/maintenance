defmodule MaintenanceJob.OtpReleases.Dev do
  @moduledoc """
  OTP Releases job.
  """
    ...
    cond do
      ...
      true ->
        fn_task_write_foo = fn ->
          ...
          # IO.inspect({__ENV__.line, otp_versions_table}, limit: :infinity, printable_limit: :infinity)

          versions = parse_otp_versions_table(otp_versions_table)
          # IO.inspect({__ENV__.line, versions}, limit: :infinity, printable_limit: :infinity)
          ...
          IO.inspect({__ENV__.line, gHReleases}, limit: :infinity, printable_limit: :infinity)
          ...
          # IO.inspect {__ENV__.line, releases, limit: :infinity, printable_limit: :infinity}
  ...
  def parse_erlang_org_downloads() do
    ...
    # IO.inspect({__ENV__.line, the_downloads}, limit: :infinity, printable_limit: :infinity)
      ...
      vsns ->
        ...
        for {vsn, key} <- results, reduce: vsns do
          map ->
            ...
            link = IO.iodata_to_binary(['https://erlang.org/download/', download])
